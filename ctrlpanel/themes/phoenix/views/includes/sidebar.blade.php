  ...
  <a class="flex gap-4 items-center ml-6 text-lg font-bold text-gray-800 dark:text-gray-200" href="{{ route('home') }}">
    @if (\Illuminate\Support\Facades\Storage::disk('public')->exists('icon.png'))
      <img
        ...
        alt="{{ config('app.name', 'ControlPanel.gg') }} Logo" class="rounded-lg w-9 h-9">
    @endif
    <span>
  ...
  <ul>
    <li class="relative px-6 py-3">
      ...
      <a class="inline-flex items-center w-full text-sm font-semibold transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200 @if (Request::routeIs('servers.*')) text-gray-800 dark:text-gray-100 @endif"
        ...
        <span
          class="ml-auto text-xs rounded py-0.5 px-2 dark:bg-gray-700/70 dark:text-gray-300 bg-gray-200/70 text-gray-700">
    ...
    @canany(['settings.discord.read', 'settings.discord.write', 'settings.general.read', 'settings.general.write',
      'settings.invoice.read', 'settings.invoice.write', 'settings.locale.read', 'settings.locale.write',
      'settings.mail.read', 'settings.mail.write', 'settings.pterodactyl.read', 'settings.pterodactyl.write',
      'settings.referral.read', 'settings.referral.write', 'settings.server.read', 'settings.server.write',
      'settings.ticket.read', 'settings.ticket.write', 'settings.user.read', 'settings.user.write',
      'settings.website.read', 'settings.website.write', 'settings.paypal.read', 'settings.paypal.write',
      'settings.stripe.read', 'settings.stripe.write', 'settings.mollie.read', 'settings.mollie.write',
      'admin.overview.read', 'admin.overview.sync', 'admin.ticket.read', 'admin.tickets.write',
      'admin.ticket_blacklist.read', 'admin.ticket_blacklist.write', 'admin.roles.read', 'admin.roles.write',
      ...
      <li class="relative px-6 py-3">
        @if (Request::routeIs('admin.ticket.index'))
          <span class="absolute inset-y-0 left-0 w-1 bg-primary-600 rounded-tr-lg rounded-br-lg"
            aria-hidden="true"></span>
        @endif
        <a class="inline-flex items-center w-full text-sm font-semibold transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200 @if (Request::routeIs('admin.ticket.index')) text-gray-800 dark:text-gray-100 @endif"
    ...
    @canany(['admin.ticket_blacklist.read', 'admin.ticket_blacklist.write'])
      <li class="relative px-6 py-3">
        @if (Request::routeIs('admin.ticket.blacklist'))
          <span class="absolute inset-y-0 left-0 w-1 bg-primary-600 rounded-tr-lg rounded-br-lg"
            aria-hidden="true"></span>
        @endif
        <a class="inline-flex items-center w-full text-sm font-semibold transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200 @if (Request::routeIs('admin.ticket.blacklist')) text-gray-800 dark:text-gray-100 @endif"
          href="{{ route('admin.ticket.blacklist') }}">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
            xmlns="http://www.w3.org/2000/svg">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
      ...
      <li class="relative px-6 py-3">
        @if (Request::routeIs('admin.roles.*'))
          <span class="absolute inset-y-0 left-0 w-1 bg-primary-600 rounded-tr-lg rounded-br-lg"
            aria-hidden="true"></span>
        @endif
        <a class="inline-flex items-center w-full text-sm font-semibold transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200 @if (Request::routeIs('admin.roles.*')) text-gray-800 dark:text-gray-100 @endif"
          ...
          <span class="ml-4">{{ __('Role Management') }}</span>
        </a>
      </li>
    @endcanany
    ...
    @canany(['settings.discord.read', 'settings.discord.write', 'settings.general.read', 'settings.general.write',
      ...
      'settings.website.read', 'settings.website.write', 'settings.paypal.read', 'settings.paypal.write',
      'settings.stripe.read', 'settings.stripe.write', 'settings.mollie.read', 'settings.mollie.write',
      'settings.mercadopago.read', 'settings.mercadopago.write'])
      <li class="relative px-6 py-3">
    ...
    @canany(['admin.users.read', 'admin.users.write', 'admin.users.suspend', 'admin.users.write.credits',
      'admin.users.write.username', 'admin.users.write.password', 'admin.users.write.role', 'admin.users.write.referal',
      'admin.users.write.pterodactyl', 'admin.servers.read', 'admin.servers.write', 'admin.servers.suspend',
      'admin.servers.write.owner', 'admin.servers.write.identifier', 'admin.servers.delete', 'admin.products.read',
      'admin.products.create', 'admin.products.edit', 'admin.products.delete'])
      <li class="relative px-6 py-3">
        <button
          ...
          <svg class="w-4 h-4 transition-transform duration-200" aria-hidden="true" fill="currentColor"
            viewBox="0 0 20 20" :class="{ 'rotate-180': isManagementMenuOpen }">
        ...
        <ul x-cloak x-cloak x-show="isManagementMenuOpen" x-transition:enter="transition-all ease-in-out duration-300"
          ...
          class="p-2 mt-2 space-y-2 overflow-hidden text-sm font-medium text-gray-600 rounded-md shadow-inner bg-gray-50 dark:text-gray-400 dark:bg-gray-900"
          aria-label="submenu">
          @canany(['admin.users.read', 'admin.users.write', 'admin.users.suspend', 'admin.users.write.credits',
            'admin.users.write.username', 'admin.users.write.password', 'admin.users.write.role',
            'admin.users.write.referal', 'admin.users.write.pterodactyl'])
            <li
          ...
          @endcanany
          @canany(['admin.servers.read', 'admin.servers.write', 'admin.servers.suspend', 'admin.servers.write.owner',
            'admin.servers.write.identifier', 'admin.servers.delete'])
            <li
          ...
          @endcanany
          @canany(['admin.products.read', 'admin.products.create', 'admin.products.edit', 'admin.products.delete'])
            <li
          ...
          @canany(['admin.coupons.read', 'admin.coupons.write'])
            <li
              class="px-2 py-1 transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200 @if (Request::routeIs('admin.coupons.*')) text-gray-800 dark:text-gray-100 @endif">
              <a class="w-full inline-block" href="{{ route('admin.coupons.index') }}">{{ __('Coupons') }}</a>
            </li>
          @endcanany
          @canany(['admin.useful_links.read', 'admin.useful_links.write'])
            <li
              ...
              <a class="w-full inline-block" href="{{ route('admin.usefullinks.index') }}">{{ __('Useful Links') }}</a>
            </li>
          @endcanany
        </ul>
      </li>
    ...
    @canany(['admin.payments.read', 'admin.logs.read'])
      <li class="relative px-6 py-3">
        <button
          ...
          <svg class="w-4 h-4 transition-transform duration-200" aria-hidden="true" fill="currentColor"
            viewBox="0 0 20 20" :class="{ 'rotate-180': isLogsMenuOpen }">
            <path fill-rule="evenodd"
              d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"
              clip-rule="evenodd"></path>
          </svg>
