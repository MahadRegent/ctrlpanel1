...
<div class="flex items-center gap-4 pt-1">
  ...
  <select name="{{ $name }}" {{ $disabled ? 'disabled' : '' }}
    @error($name) {!! $attributes->merge([
        ...
        {!! $attributes->merge([
            'class' =>
                'block w-full text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-primary-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray rounded-md shadow-sm border-gray-300 focus:ring focus:ring-primary-200 focus:ring-opacity-50',
        ]) !!} @enderror>
    {{ $slot }}
  </select>
