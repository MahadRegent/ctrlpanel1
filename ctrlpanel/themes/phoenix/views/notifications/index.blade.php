  ...
  <div class="container px-6 mx-auto grid">
    ...
    <div class="mb-4 flex justify-between py-6">
      ...
      <a href="{{ route('notifications.readAll') }}"
        class="px-3 py-1 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-primary-600 border border-transparent rounded-md focus:shadow-outline-purple
         active:bg-primary-600 hover:bg-primary-700 focus:shadow-outline-purple  focus:outline-none">{{ __('Mark all as Read') }}</a>
    </div>
      ...
      @foreach ($notifications as $notification)
        <div class="min-w-0 p-4 bg-white rounded-lg shadow-sm dark:bg-gray-800 mb-4 flex items-center  justify-between">
          <h4 class="font-semibold text-gray-600 dark:text-gray-300">
            <a class="{{ $notification->read() ? 'text-primary-600' : 'dark:text-primary-700 text-primary-500' }}"
