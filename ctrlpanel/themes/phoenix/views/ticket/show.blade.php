      ...
      <div class="min-w-0 p-4 bg-white rounded-lg shadow-sm dark:bg-gray-800">
        <div class="card-body">
          <div class="ticket-info space-y-2">
            @if (!empty($server))
              <p><b>{{ __('Server') }}:</b> <a href="{{ $pterodactyl_url }}/server/{{ $server->identifier }}"
            ...
            <p>
              @if ($ticket->status === 'Open' || $ticket->status === 'Reopened')
                <b>{{ __('Status') }}:</b> <span
                  class="px-2 py-1 text-xs font-semibold leading-tight rounded-full text-green-700 bg-green-100 dark:bg-green-500/20 dark:text-green-500">{{ __($ticket->status) }}</span>
            ...
            <div><b>{{ __('Created on') }}:</b>
              <x-tooltip :content="$ticket->created_at->isoFormat('LLL')" pos="top" class="inline">
            ...
            <div
              ...
              <div
                ...
                <span class="badge badge-primary">
                  <x-tooltip :content="$ticket->created_at->isoFormat('LLL')" pos="bottom" class="inline">
                    {{ $ticket->created_at->diffForHumans() }}
                  </x-tooltip>
                </span>
            ...
            @foreach ($ticketcomments as $ticketcomment)
              <div
                ...
                <div
                  ...
                  <span class="badge badge-primary">
                    <x-tooltip :content="$ticketcomment->created_at->isoFormat('LLL')" pos="bottom" class="inline">
              ...
              <form action="{{ route('ticket.reply') }}" method="POST" class="form reply-form">
                ...
                <label class="block text-sm">
                  ...
                  <textarea id="ticketcomment" name="ticketcomment" type="text" rows="8"
                    class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-primary-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray rounded-md shadow-sm border-gray-300 focus:ring focus:ring-primary-200 focus:ring-opacity-50"></textarea>
                </label>
                <button type="submit"
