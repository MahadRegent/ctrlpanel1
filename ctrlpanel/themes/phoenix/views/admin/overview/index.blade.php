...
@section('content')
  @php($ticketSettings = app(App\Settings\TicketSettings::class))

  <x-container title="Admin Overview">
    @if (Storage::get('latestVersion') && config('app.version') < Storage::get('latestVersion'))
      <x-alert type="danger" title="Version Outdated">
        {{ __('You are running on') }} v{{ config('app.version') }}-{{ trim(config('BRANCHNAME')) }},
        {{ __('The latest Version is') }} v{{ Storage::get('latestVersion') }}</br>
        <a class="underline" href="https://CtrlPanel.gg/docs/Installation/updating">{{ __('Consider updating now') }}</a>
      </x-alert>
    @endif
    ...
    <div class="grid gap-6 mb-8 md:grid-cols-2 xl:grid-cols-4">
      ...
      <div class="flex items-center p-4 bg-white rounded-lg shadow-sm dark:bg-gray-800">
        ...
        <div class="w-full">
          <div class="flex justify-between ">
            <span class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-400">{{ __('Servers') }}</span>

            <x-tooltip content="Active/Total" pos="left" class="border-none">
              <x-icon icon="mingcute:information-line" height="20" width="20"
                class="mr-2 flex items-center cursor-pointer"></x-icon>
            </x-tooltip>
          </div>
      ...
      <div class="flex items-center p-4 bg-white rounded-lg shadow-sm dark:bg-gray-800">
        ...
        <div class="w-full">
          <div class="flex justify-between ">
            <span class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-400">{{ __('Users') }}</span>
    ...
    <div class="grid gap-6 mb-8 @if ($ticketSettings->enabled) md:grid-cols-3 @else md:grid-cols-2 @endif grid-cols-1">
      ...
      <div class="w-full overflow-hidden col-span-2 md:col-span-1">
        ...
        <div class="flex flex-col p-4 bg-white rounded-lg shadow-sm dark:bg-gray-800">
          ...
          <div class="mb-2">
            ...
            <p class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-400">
              {{ __('Github of Controlpanel.gg project! It\'s open-source!') }}
            </p>
          </div>
          <div class="mb-2">
            <a class="underline text-gray-700 dark:text-gray-200 font-semibold" target="__blank"
                ...
                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                  @foreach ($tickets as $ticket_id => $ticket)
                    <tr class="text-gray-700 dark:text-gray-400">
                      <td class="px-4 py-3 text-sm text-primary-600 underline">
                        <a class=""
                          href="{{ route('admin.ticket.show', ['ticket_id' => $ticket_id]) }}">{{ $ticket->title }}</a>
                      </td>
                      <td class="px-4 py-3 text-sm text-primary-600 underline">
      ...
      @endif
      <div class="w-full
      overflow-hidden col-span-2 md:col-span-1">
        <div class="mb-4 flex justify-between ">

          <h2 class="text-xl font-semibold text-gray-700 dark:text-gray-200">
            {{ __('Pterodactyl') }}
          </h2><a href="{{ route('admin.overview.sync') }}"
            class="px-3 py-1 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-primary-600 border border-transparent rounded-md active:bg-primary-600 hover:bg-primary-700 focus:outline-none focus:shadow-outline-purple">{{ __('Sync') }}</a>
        </div>
            ...
            <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
              ...
              <tr>
                <td colspan="2" class="px-4 py-3 text-sm">
                  {{ __('Last updated :date', ['date' => $syncLastUpdate]) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
          ...
          <table class="w-full whitespace-no-wrap">
            <thead>
              <tr
                ...
                <th class="px-4 py-3">{{ $credits_display_name . ' ' . __('Usage') . ' (' . __('per month') . ')' }}
                </th>
              </tr>
            </thead>
            <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
            ...
            <tfoot class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
              <tr class="text-gray-500 dark:text-gray-400">
                ...
                <td class="px-4 py-3">
                  {{ __('Total') }} ({{ __('active') }}/{{ __('total') }}):</span>
                </td>
                <td class="px-4 py-3">{{ $counters['servers']->active }}/{{ $counters['servers']->total }}</td>
                <td class="px-4 py-3">{{ $counters['totalUsagePercent'] }}%</td>
                <td class="px-4 py-3">{{ $counters['earnings']->active }}/{{ $counters['earnings']->total }}</td>
              </tr>
            </tfoot>
          </table>
        </div>
